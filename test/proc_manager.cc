/****************** -*- Mode: C++; Indent: Inktomi4 -*- *********************
   ...
   $Id: proc_manager.cc,v 1.3 2006-03-31 04:27:58 legalov Exp $
...
void PM_output_log_line(const char* start, const char* end,
...
void PM_Note(const char* format_str, ...) {
...
void PM_Warning(const char* format_str, ...) {
...
void PM_Error(const char* format_str, ...) {
...
void PM_Fatal(const char* format_str, ...) {
...
ProcRecord* find_instance(const char* name) {
...
void add_instance(ProcRecord* pr) {
...
void remove_instance(ProcRecord* pr) {
...
void AcceptHandler::start(int port) {
...
void AcceptHandler::handle_accept(s_event_t event, void* data) {
...
void NetCmdHandler::process_start_cmd(RafCmd* cmd) {
    ...
    Debug("process", "Start cmd for instance %s is '%s%s%s%s%s'",
	  ...
	  (proc_record->tmp_start_args) ? " " : "",
	  (proc_record->tmp_start_args) ? proc_record->tmp_start_args : "");
 
    proc_record->start_process();
    send_raf_resp(cmd, 0, "started '%s'", instance_name);
}

void NetCmdHandler::process_stop_cmd(RafCmd* cmd) {
	...
	if (tmp->pid < 0) {
	    ...
	    ink_debug_assert(tmp->proc_status == PROC_STATUS_RUNNING ||
	    ...
	    if (r < 0) {
		send_raf_resp(cmd, 1, "kill failed for '%s'", instance_name,
			      strerror(errno));
	    } else {
...
void NetCmdHandler::process_create_cmd(RafCmd* cmd) {
...
void NetCmdHandler::create_config_and_install() {
	...
	if (create_prec->package_name) {
	    ...
	    remove_instance(create_prec);
	    create_prec = NULL;
	    send_raf_resp(raf_cmd, 1, "no package instantitor specified for '%s'",
			  create_prec->instance_name);
	    return;
	}

	if (installer_name == NULL) {
	    remove_instance(create_prec);
	    send_raf_resp(raf_cmd, 1, "could not find instantitor for '%s'",
			  create_prec->instance_name);
	} else {
	    poll_interest = POLL_INTEREST_NONE;
	...
	send_raf_resp(raf_cmd, 0, "created instance '%s'",
		      create_prec->instance_name);
    }
...
void NetCmdHandler::handle_create_rundir_rm(s_event_t event, void* data) {
	...
	case SEVENT_RMDIR_FAILURE:
	{
	    ...
	    if (rmsg != NULL) {
		PM_Error("%s %s for %s : %s", rmsg, create_prec->run_dir,
			 create_prec->instance_name, strerror(err));
		create_prec = NULL;
		remove_instance(create_prec);
...
void NetCmdHandler::handle_create_completion(s_event_t event, void* data) {
...
void NetCmdHandler::process_destroy_cmd(RafCmd* cmd) {
...
void NetCmdHandler::process_install_cmd(RafCmd* cmd) {
...
void NetCmdHandler::handle_install_completion(s_event_t event, void* data) {
    ...
    if (r < 0) {
	PM_Error("install failed for %s - can not access unpacked dir: %s",
		 package_name, strerror(errno));
	send_raf_resp(raf_cmd, 1, "install failed for %s - can not access unpacked dir: %s",
		      package_name, strerror(errno));
	return;
    }
    ...
    if (r < 0) {
	PM_Error("install failed for %s - can not create active link: %s",
		 package_name, strerror(errno));
	send_raf_resp(raf_cmd, 1, "install failed for %s - can not create active link: %s",
		 package_name, strerror(errno));
	return;
    }
...
char* NetCmdHandler::generate_anon_instance_name() {
...
char* NetCmdHandler::setup_anon_run_dir() {
...
char* NetCmdHandler::find_anon_binary_path(const char* binary) {
...
int check_anon_binary(const char* binary) {
    int r;
    do {
...
void NetCmdHandler::process_run_cmd(RafCmd* cmd) {
...
void NetCmdHandler::process_get_file_cmd(RafCmd* cmd) {
...
void NetCmdHandler::process_stat_file_cmd(RafCmd* cmd) {
...
void NetCmdHandler::process_put_file_cmd(RafCmd* cmd) {
...
void NetCmdHandler::handle_get_file(s_event_t event, void* data) {
	...
	if (r <= 0) {
	    PM_Error("read for raf 'get_file' failed : %s",
	...
	if (r < 0) {
	    if (r != EAGAIN) {
		PM_Error("write for raf 'get_file' failed : %s",
			 strerror(errno));
		delete this;
		return;
	    }
...
void NetCmdHandler::handle_put_file(s_event_t event, void* data) {
	...
	if (r <= 0 ) {
	    PM_Error("read failed for put_file : %s",
		     (r == 0) ? "eos" : strerror(errno));
	    exit_mode = RAF_EXIT_CONN;
	    close(put_fd);
	    put_fd = -1;
	    send_raf_resp(raf_cmd, 1, "read failed : %s",
			  (r == 0) ? "eos" : strerror(errno));
	    return;
	} else {
	    cmd_buffer->fill(r);
...
void NetCmdHandler::process_take_pkg_cmd(RafCmd* cmd) {
	...
	if (mkdir < 0) {
	    ...
	    send_raf_resp(cmd, 1, "failed to create dir %s for '%s': %s",
			  tmp, package_name, strerror(errno));
	    return;
	}
    }
	...
	send_raf_resp(cmd, 1, "failed to create pkg_file %s: %s",
		      tmp, strerror(errno));
	return;
    }
...
void NetCmdHandler::handle_take_pkg(s_event_t event, void* data) {
	...
	if (r == 0 || errno != EAGAIN) {
	    exit_mode = RAF_EXIT_CONN;  // Don't try to continue after a bad take_pkg
	    PM_Error("read of pkg_file for %s failed: %s",
		     (*raf_cmd)[2], strerror(errno));
	    send_raf_resp(raf_cmd, 1, "read of pkg_file failed: %s",
			  strerror(errno));
	    return;
	}
    } else {
	...
	if (r < 0) {
	    if (r != EAGAIN) {
		exit_mode = RAF_EXIT_CONN;  // Don't try to continue after a bad take_pkg
		PM_Error("write to pkg_file for %s failed: %s",
			 (*raf_cmd)[2], strerror(errno));
		send_raf_resp(raf_cmd, 1, "write to pkg_file failed: %s",
			      strerror(errno));
		return;
	    }
	} else {
...
void NetCmdHandler::process_show_pkgs_cmd(RafCmd* cmd) {
	...
	snprintf(active_link, 1023, "%s/install/%s/active",
		     stuff_dir, de->d_name);
	active_link[1023] = '\0';
	...
	if (r < 0) {
	    Warning("failed to read active link for package %s : %s",
...
void NetCmdHandler::process_arch_cmd(RafCmd* cmd) {
...
void NetCmdHandler::process_isalive_cmd(RafCmd* cmd) {
    send_raf_resp(raf_cmd, 0, "alive");
}

void NetCmdHandler::process_alloc_port(RafCmd* cmd) {
...
void NetCmdHandler::process_query_cmd(RafCmd* cmd) {
...
bool NetCmdHandler::handle_query_process_info(Tokenizer* slash_tok, RafCmd* raf_resp) {
...
void NetCmdHandler::output_query_process_info(ProcRecord* pr,
					      const char* q_proc_value,
					      RafCmd* raf_resp,
					      int* next_index) {
	...
	output_query_process_str(pr, q_proc_value, raf_resp,
				 next_index, "<unknown attribute>");
    }
}

void NetCmdHandler::output_query_process_int(ProcRecord* pr,
					     const char* q_proc_value,
					     RafCmd* raf_resp,
					     int* next_index,
					     int value) {
...
void NetCmdHandler::output_query_process_str(ProcRecord* pr,
					     const char* q_proc_value,
					     RafCmd* raf_resp,
					     int* next_index,
					     const char* value) {
...
void NetCmdHandler::process_log_get_cmd(RafCmd* cmd) {
...
void NetCmdHandler::fill_log_resp_buffer(int eof) {
...
void NetCmdHandler::handle_send_log(s_event_t event, void* data) {
	...
	if (r < 0) {
	    ...
	    send_raf_resp(raf_cmd, 1, "(done) error: read of log file failed: %s",
			  strerror(errno));
	    return;
	} else if (r == 0) {
	    close(log_fd);
...
void NetCmdHandler::process_exit_cmd(RafCmd* cmd) {
    exit_mode = RAF_EXIT_CONN;
    send_raf_resp(cmd, 0, "exiting...");
}

void NetCmdHandler::process_shutdown_cmd(RafCmd* cmd) {
    exit_mode = RAF_EXIT_PROCESS;
    send_raf_resp(cmd, 0, "shutdown...");
}

void NetCmdHandler::handle_execute_stop(s_event_t event, void* data) {
	...
	case SEVENT_TIMER:
	{
		...
		// Kill cmd failed
		proc_watch->cancel();
		send_raf_resp(raf_cmd, 1, "kill failed for '%s'", (*raf_cmd)[2],
			      strerror(errno));
	    } else {
...
void LogHandler::output_log_line(char* start, char* end) {
    PM_output_log_line(start, end, proc_record->instance_name, stream_id);
}

void LogHandler::handle_log_data(s_event_t event, void* data) {
    ...
    if (r < 0) {
	if (errno != EAGAIN) {
	    PM_Error("Could not read %s from %s : %s", proc_record->instance_name,
	...
	*(read_buffer->end()) = '\0';
	...
	Debug("log_data", "%s(%s): %s", proc_record->instance_name,
	      stream_id, read_buffer->start());
...
ProcPortBinding::~ProcPortBinding() { 
    ...
    if (bind_type == PROC_PORT_BIND_STRING &&
	value.str != NULL) {
	free(value.str);
	value.str = NULL;
    }
...
const char* ProcRecord::init_managed_proc(const char* iname) {
...
rundir_result_t ProcRecord::init_managed_rundir() {
	...
	if (r == 0) {
	    ...
	    if (rmsg != NULL) {
		PM_Error("%s %s for %s : %s", rmsg, run_dir,
			 instance_name, strerror(err));
		return PRR_ERROR;
	    } else {
		return PRR_CONTINUE;
	    }
...
char* ProcRecord::find_installer() {
	...
	while ((dp = readdir(d)) != NULL) {
		...
		sprintf(de, "%s%s%s",
			...
			dp->d_name);
		closedir(d);
		Debug("install", "[ProcRecord::find_installer] found %s for %s",
		      de, instance_name);
...
S_Action* ProcRecord::run_installer(S_Continuation* cont, const char* installer_name) {
...
void ProcRecord::process_installer_cmd_line(const char* val_start, const char* val_end) {
    ...
    Debug("install", "%s: setting start_args to '%s'", instance_name,
	  start_args ? start_args : "<NULL>");
}

void ProcRecord::process_installer_env_vars(const char* val_start, const char* val_end) {
	...
	Debug("install", "%s: adding env var %s",
	      instance_name, env_vars[i]);
...
void ProcRecord::process_installer_port_binding(const char* val_start, const char* val_end) {
	...
	Debug("install", "%s: adding port binding %s:%d",
	      instance_name, pb->name, pb->value.port);
...
void ProcRecord::process_installer_ports_used(const char* val_start, const char* val_end) {
...
void ProcRecord::process_installer_output(sio_buffer* buf) {
...
const char* ProcRecord::create_pipe(int* pipe_array, const char* id) {
...
void ProcRecord::set_nonblock(int fd, const char* id) {
    int r;
    do {
...
const char* ProcRecord::start_process() {
	...
	for (int i = 3; i < 1024; i++) {
		...
		(proc_category != PC_INSTALL_PROCESS  &&
		 (i == install_in_pipe[0] || i == install_out_pipe[1]))) {
		close(i);
	    }
	}
	...
	if (r < 0) {
	    PM_Fatal("%s : could not change to run dir %s : %s",
		  instance_name, chd, strerror(errno));
	}
...
S_Action* ProcRecord::set_watch(S_Continuation* c) {
...
void ProcRecord::notify_watchers() {
...
int ProcRecord::write_config(const char* config) {
    ...
    if (config_fd < 0) {
	PM_Error("Unable to open config blob file %s : %s", config_file,
	      strerror(errno));
	return 1;
    }
	...
	if (r < 0) {
		...
		PM_Error("Unable to write to config blob file %s : %s",
...
void InstallerSendInput::start_send(InstallerHandler* master_arg,
				    ProcRecord* p_rec, int fd_arg) {
    master = master_arg;
    this->fd = fd_arg;
...
void InstallerSendInput::handle_send(s_event_t event, void* data) {
	...
	if (r == EAGAIN) {
	    ...
	    PM_Warning("write to %s failed : %s",
		    master->installer_rec->instance_name,
		    strerror(errno));
	    ...
	    PM_Warning("write pipe to %s closed",
		    master->installer_rec->instance_name);
	    master->set_send_status(-1);
    } else {
...
void InstallerSendInput::add_pair(const char* name, const char* value) {
...
void InstallerReadOutput::start_read(InstallerHandler* master_arg, int fd_arg) {
...
void InstallerReadOutput::handle_read(s_event_t event, void* data) {
	...
	if (errno == EAGAIN) {
	    ...
	    PM_Warning("read from %s failed : %s",
		    master->installer_rec->instance_name,
		    strerror(errno));
...
void InstallerHandler::init(ProcRecord* prec, int input_fd, int output_fd) {
...
void InstallerHandler::watch_installer(s_event_t event, void* data) {
	...
	case SEVENT_PROC_STATE_CHANGE:
		...
		PM_Warning("non-zero exit status from %s",
		    installer_rec->instance_name);
		handle_install_error();
...
void InstallerHandler::handle_install_error() {
...
void InstallerHandler::handle_install_success() {
...
void InstallerHandler::set_read_status(int status) {
    read_status = status;
	...
	*(output_reader->read_buffer->end()) = '\0';
	Debug("install", "%s params (%d): %s",
	      installer_rec->instance_name, output_reader->read_buffer->read_avail(),
	      output_reader->read_buffer->start());
    }
...
void InstallerHandler::set_send_status(int status) {
    send_status = status;
}
...
S_Action* EventForwarder::forward_event(S_Continuation* c, s_event_t e, void* d) {
    cont = c;
    event = e;
    data = d;
...
void EventForwarder::handle_event(s_event_t e, void* d) {
...
void RecursiveRmDir::handle_rm_complete(s_event_t e, void* d) {
...
S_Action* RecursiveRmDir::do_remove_dir(S_Continuation* cont,
					const char* dir_to_rm, const char* tag) {
    ...
    if (dir_to_rm == NULL || dir_to_rm[0] == '\0' ||
	rm_bin_path == NULL || rm_bin_path == '\0')  {
    ...
    if (r < 0) {
	PM_Error("Unable to find old instance dir %s : %s",
		 dir_to_rm, strerror(errno));
	SIO::schedule_in(this, 0);
	return &action;;
    }
...
char* RecursiveRmDir::rm_bin_path = NULL;

void find_rm_bin() {
    const char* check_locations[] = {
	"/bin/rm",
	"/usr/bin/rm",
	"/usr/local/bin/rm",
	NULL
    ...
    const char** tmp = check_locations;
...
static int exit_signal_received = 0;

void process_child_exit(pid_t pid, int status) {

    bool proc_found = false;
    ProcRecord* cur = process_list.head;
	...
	if (cur->pid == pid) {
		...
		if (cur->proc_status != PROC_STATUS_STOPPING) {
		    PM_Warning("Child process %s (pid %d) killed by signal %d",
			       cur->instance_name, pid, WTERMSIG(status));
		    cur->proc_status = PROC_STATUS_FAIL;
		} else {
		    cur->proc_status = PROC_STATUS_STOPPED;
		}
...
void init_log_stuff() {
	...
	int port = 12301;
	char* colon = strchr(log_collator, ':');
	if (colon) {
	    *colon = '\0';
	    port = atoi(colon + 1);
	    if (port == 0) {
...
void init_dir_stuff() {
	...
	if (stuff_dir[0] != '/') {
	    // FIX - buffer overrun :-(
	    char new_stuff_dir[1024];
	    char* r = getcwd(new_stuff_dir, 1024);
	    if (r == NULL) {
		PM_Fatal("getcwd failed: %s", strerror(errno));
	    }
	    int len = strlen(new_stuff_dir);
    ...
    int error_code;
    const char* rmsg = create_or_verify_dir(stuff_dir, &error_code);
    if (rmsg) {
...
void manage_lockfile() {
    ...
    int r = lockfile->Get(&holding_pid);
...
void setup_signals() {
    ...
    int ignore_sigs[5] = { SIGPIPE, SIGHUP, SIGUSR1, SIGUSR2, SIGALRM };
...
void setup_proc_group() {
    pid_t our_proc_group = setsid();
...
static void redirect_stdout_stderr() {
    ...
    if (out_fd < 0) {
	Fatal("unable to open %s for stdout redirection : %s",
	      tmp, strerror(errno));
    }
