/****************** -*- Mode: C++; Indent: Inktomi4 -*- *********************
   ...
   $Id: test_group.cc,v 1.2 2003-06-01 18:38:29 re1 Exp $
...
int process_test_entry(InkHashTable* h, const char* tag,
		       RafCmd* line_el, int line_num) {

    if (line_el->length() < 3) {
	TE_Error("insufficent arguments to '%s' on line %d of test group file",
		 tag, line_num);
	return -1;
    }
...
int process_group_data(sio_buffer* input, sio_buffer* line_buffer,
		       int* line_num, bool eof) {
	...
	if (comps.length() > 0) {
		...
		if (strcasecmp(comps[0], "test_case") == 0) {
		    process_test_entry(test_case_hash, "test_case",
				       &comps, *line_num);
		} else if (strcasecmp(comps[0], "test_group") == 0) {
		    process_test_entry(test_group_hash, "test_group",
				       &comps, *line_num);
		} else {
		    TE_Error("unknown identifier '%s' on line %d of test group file",
			     comps[0], *line_num);
		}
	    }
	}
...
int load_group_file(const char* filename) {
    ...
    if (fd < 0) {
	TE_Error("Failed to open group file %s : %s",
	...
	if (r < 0) {
	    TE_Error("Read from test group file failed : %s",
		     strerror(errno));
	    close(fd);
	    return -1;
	} else if (r == 0) {
...
int lookup_test_case(const char* name, test_case* tcase) {
