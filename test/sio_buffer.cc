/****************** -*- Mode: C++; Indent: Inktomi4 -*- *********************
   ...
   $Id: sio_buffer.cc,v 1.2 2003-06-01 18:38:29 re1 Exp $
...
void sio_buffer::init_buffer(int size) {
...
int  sio_buffer::expand_to(int size) {
...
int sio_buffer::fill(const char* data, int data_len) {
...
int sio_buffer::read_avail() {
    int r = data_end - data_start;
    ink_debug_assert(r >= 0);
    return r;
}

int sio_buffer::write_avail() {
    int r = raw_end - data_end;
    ink_debug_assert(r >= 0);
    return r;
}

void sio_buffer::reset() {
    data_start = data_end = raw_start;
}

void sio_buffer::consume(int n) {
    int ravail = read_avail();
...
char* sio_buffer::memchr(int c, int len, int offset) {

    int ravail = read_avail();
    ink_debug_assert((len == INT_MAX  && offset <= ravail) ||
		     len  <= ravail - offset);
...
int sio_buffer::read(char* buf, int len) {
...
char* sio_buffer::memcpy(char* buf, int len, int offset) {
    int ravail = offset;
