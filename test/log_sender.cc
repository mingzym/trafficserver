/****************** -*- Mode: C++; Indent: Inktomi4 -*- *********************
   ...
   $Id: log_sender.cc,v 1.2 2003-06-01 18:38:29 re1 Exp $
...
void LogSender::start_to_file(const char* file_arg) {
    ...
    if (this->fd < 0) {
	Fatal("Unable to open log file %s : %s", log_file_name,
	      strerror(errno));
    }
...
void LogSender::start_to_net(unsigned int ip, int port) {
...
void LogSender::handle_output(s_event_t event, void* data) {
...
void LogSender::add_to_output_log(const char* start, const char* end) {
...
const char* LogSender::roll_log_file(const char* roll_name) {
	...
	if (r < 0) {
	    Error("[LogSender::roll_log_file] failed to unlink new name: %s",
		  strerror(errno));
    ...
    if (r < 0) {
	Error("[LogSender::roll_log_file] failed to create link : %s",
	      strerror(errno));
	return "link failed";
    }
    ...
    if (r < 0) {
	Error("[LogSender::roll_log_file] failed to unlink old file: %s",
	      strerror(errno));
    ...
    if (this->fd < 0) {
	Fatal("[LogSender::roll_log_file] Unable to open new log file %s : %s",
