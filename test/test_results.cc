/****************** -*- Mode: C++; Indent: Inktomi4 -*- *********************
   ...
   $Id: test_results.cc,v 1.2 2003-06-01 18:38:30 re1 Exp $
...
void TestResult::start(const char* name_arg) {

    test_case_name = strdup(name_arg);
    time_start = time(NULL);
}

void TestResult::build_output_file_name(const char* base,
					const char* ext) {
    ...
    sprintf(output_file, "%s/%s/%s.%s", save_results_dir,
	    test_run_results->run_id_str, base, ext);
}
...
void TestRunResults::start(const char* tname, const char* uname, const char* bid) {
	...
	if (r) {
	    TE_Error("Could not create save dir : %s : %s",
		     tmp, strerror(errno));
	    save_results = 0;
	}
    }
...
int TestRunResults::output_summary_html() {

    char summary_file[1024];
    snprintf(summary_file, 1023, "%s/%s/index.html",
		 save_results_dir, run_id_str);
    summary_file[1023] = '\0';
    ...
    if (fd < 0) {
	TE_Error("Failed to create summary output file %s : %s",
    ...
    int r = 0;
    if (result_msg != NULL) {
	TE_Error("Failed to write to summary file %s : %s",
		 summary_file, strerror(errno));
	r = -1;
    }
...
void TestRunResults::build_summary_html(sio_buffer* output) {
	...
	"<th width=\"15%\"> Errors </th> <th width=\"15%\"> Warnings </th> <th width=\"15%\"> "
	"Duration </th></tr>\n";
...
void TestRunResults::send_final_tinderbox_message() {
...
void TestRunResults::cleanup_results(bool print) {
...
TestResult* TestRunResults::new_result() {
...
void TestRunResults::build_tinderbox_message_hdr(const char* status, time_t now,
						 sio_buffer* output) {

    Debug("tinderbox", "Build tinderbox msg with status %s", status);

    const char admin_hdr[] = "tinderbox: administrator : ";
    const char admin_hdr_end[] = "@inktomi.com\n";
...
int TestRunResults::post_tinderbox_message(sio_buffer* hdr, sio_buffer* body) {
