/****************** -*- Mode: C++; Indent: Inktomi4 -*- *********************
   ...
   $Id: test_exec.cc,v 1.2 2003-06-01 18:38:29 re1 Exp $
...
void TE_output_log_line(const char* start, const char* end,
...
void InstanceRecord::add_port_binding(const char* name, const char* value) {
    ...
    Debug("ports", "%s: Adding port binding %s => %s",
	  instance_name, name, value);
}

const char* InstanceRecord::get_port_binding(const char* name) {
...
const char* HostRecord::lookup_package(const char* pkg_name) {
...
void HostRecord::update_package_entry(const char* pkg_name, const char* new_pkg, int new_pkg_len) {
    InkHashTableValue val = NULL;
    int r = ink_hash_table_lookup(package_table, (char*) pkg_name, &val);
...
int send_file(int to_fd, int from_fd, int bytes,
	      const char* filename, const char* to_id) {
	...
	if (r < 0 || r == 0) {
	    TE_Error("[send_file] read from file %s failed: %s",
	...
	const char* rmsg =
	    write_buffer(to_fd, &input_file_buffer, &timeout_ms);

	if (rmsg) {
	    TE_Error("send_file %s to %s failed: %s",
		     filename, to_id, strerror(errno));
	    return -1;
	}
    }
...
int push_package(HostRecord* hrec, const char* pkg_name,
		 const char* pkg_filename, const char* pkg_filepath) {
    ...
    // Send the file
    if (send_file(hrec->fd, pkg_fd, stat_info.st_size,
		  pkg_filename, hrec->hostname) < 0) {
	goto CLEANUP;
    }
	...
	TE_Error("raf cmd take_pkg %s to %s failed: %s",
		 pkg_filename, hrec->hostname, rmsg);
	goto CLEANUP;
    }
...
int do_package_management(HostRecord* hrec, const char* pkg_name) {
...
int pm_base_cmd(HostRecord* hrec, RafCmd* request,
		RafCmd* response, const char* cmd,
		const char* instance_name, char** args) {
...
HostRecord* find_or_setup_host(const char* host_name, const char* instance_name) {
	...
	if (r > 0) {
	    ...
	    hostname_sub.fill("", 1);
	    Debug("subs", "Substituted hostname %s to %s for %s",
		  host_name, hostname_sub.start(), instance_name);
...
int pm_create_instance(const char* instance_name, const char* host_name, char** args) {
	...
	if (do_package_management(host_rec, pkg_name) != 0) {
	    TE_Error("Package push of %s for instance %s failed",
...
static char* pm_run_internal(HostRecord* host_rec, const char* binary, const char* args,
			     const char* master_instance, int timeout) {
...
int add_to_log(const char* log_line) {
    ...
    TE_output_log_line(log_line, log_line + len, 
		       "test_script", "log");
...
int set_var_value(const char* var, const char* var_value) {
...
// 
//  CALLER FREES return value 
static char* construct_instance_file_path(InstanceRecord* irec, const char* file) {
...
// 
//  CALLER FREES return value 
static char* construct_script_file_path(const char* file) {
...
static void set_cur_script_path(const char* default_script_dir, const char* script_name) {
...
int put_instance_file_raw(const char* instance, const char* relative_path,
    ...
    if (fd < 0) {
	TE_Warning("put_instance_file_raw open failed : %s : %s",
    ...
    if (r < 0) {
	close(fd);
	TE_Warning("put_instance_file_raw fstat failed : %s : %s",
		   src, strerror(errno));
	return -1;
    }
    ...
    // Send the file
    if (send_file(irec->host_rec->fd, fd, stat_info.st_size,
		  relative_path, irec->instance_name) < 0) {
	goto CLEANUP;
    }

    // Read the raf response
    rmsg = read_raf_resp(irec->host_rec->fd, irec->host_rec->read_buffer, 
			 &response, &timeout_ms);

    if (rmsg || response.length() < 2 || atoi(response[1]) != 0) {
	TE_Error("raf cmd put_file %s to instance %s failed : %s",
		 src, instance, rmsg ? rmsg : response[2]);
	goto CLEANUP;
    }
...
int put_instance_file_subs(const char* instance, const char* relative_path,
    ...
    if (fd < 0) {
	TE_Warning("put_instance_file_subs open failed : %s : %s",
		   src_path, strerror(errno));
	free(src_path);
	return -1;
    ...
    if (r < 0) {
	close(fd);
	TE_Warning("put_instance_file_subs fstat failed : %s : %s",
		   src, strerror(errno));
	return -1;
    }
    ...
    const char* r_msg =
	read_to_buffer(fd, &file_buffer, stat_info.st_size, &eof, &timeout_ms);

    close(fd);

    if (r_msg || eof != 0) {
	TE_Error("put_instance_file_subs read failed : %s",
		 r_msg ? r_msg : "eof");
	return -1;
    }
    ...
    r = do_substitutions(file_buffer.start(), file_buffer.read_avail(),
			 &sub_buffer, &sub_errors);

    if (sub_errors > 0) {
	TE_Warning("put_instance_file_subs %s had %d substitution errors",
		   src, sub_errors);
    }
    ...
    if (rmsg || response.length() < 2 || atoi(response[1]) != 0) {
	TE_Error("raf cmd put_file_subs %s to instance %s failed : %s",
		 src, instance, rmsg ? rmsg : response[2]);
	goto CLEANUP;
    }
...
int query_process_int(const char* instance, const char* field, int* result) {
    ...
    char** resultv =
	raf_proc_manager(instance, "query", queryv);
...
int wait_for_server_port(const char* instance, const char* port_str, int timeout_ms) {
    ...
    if (port <= 0) {
	TE_Warning("[wait_for_server] Could not resolve %s port %s",
		   instance, port_str);
	return -1;
    }
...
char* get_instance_file(const char* instance, const char* file) {
    ...
    int timeout_ms = 60000;
    const char* r_msg = 
	send_raf_cmd(irec->host_rec->fd, &raf_req, &timeout_ms);
    
    if (r_msg) {
	TE_Error("send of raf cmd 'get_file' to %s failed: %s",
		 irec->host_rec->hostname, r_msg);
	return NULL;
    }

    // Read the Raf response
    sio_buffer resp_buffer;
    r_msg = read_raf_resp(irec->host_rec->fd, &resp_buffer, &raf_resp,
			  &timeout_ms);

    // Check the raf the response to see if was successful
    if (r_msg) {
	TE_Error("read of raf resp to 'get_file' from %s failed: %s",
		 irec->host_rec->hostname, r_msg);
	return NULL;
    }
    ...
    if (local_fd < 0) {
	TE_Error("could not create %s for raf cmd 'get_file' : %s",
	...
	if (read_bytes_left > 0) {
	    ...
	    r_msg = read_to_buffer(irec->host_rec->fd, &resp_buffer, act_on,
				   &eof, &timeout_ms);

	    if  (eof || r_msg) {
		TE_Error("file xfer for raf cmd 'get_file' from %s failed: %s",
			 instance, (eof != 0) ? "eof" : r_msg);
		xfer_failed = true;
		break;
	    } else {
	...
	if (local_fd < 0) {
	    ...
	    if (r_msg) {
		xfer_failed = true;
		TE_Error("file write for raf cmd 'get_file' from %s failed: %s",
			 instance, r_msg);
		close(local_fd);
		local_fd = -1;
...
//
static char** build_raf_err_argv(const char* err_str) {
...
//     and strings pointed to by return value
//
static char** raf_host_port(unsigned int ip, int port,
...
char** raf_proc_manager(const char* instance_name, const char* raf_cmd, char** raf_args) {
    ...
    if (irec) {
	return_value = raf_host_port(irec->host_rec->ip, irec->host_rec->port,
				     raf_cmd, raf_args);
    }   else {
	err_str = "No such instance";
    }
...
void stop_and_destroy_all_instances() {
...
void shutdown_log_collator() {
	...
	TE_Error("failed to shutdown log collator");
	       ...
	       strcmp(response[1], "0") != 0) {
	TE_Error("collator shutdown shutdown cmd failed: %s %s",
		 (response.length() >= 2) ? response[1] : "?",
		 (response.length() >= 3) ? response[1] : "?");
    } else {
...
int output_substitution(sio_buffer* output_buffer, const char* modifier_str,
	...
	while (*modifier_str != '\0') {
		...
		case 'r':
			...
			break;
		default:
		    TE_Warning("Bad modifier '%c' on substitution %s ignored",
			       *modifier_str, sub_name);
		    break;
	...
	if (modifiers & SUB_MOD_IP_ADDR) {
		...
		TE_Warning("hostname lookup for substition of '%s' failed : %s",
			   sub_name, strerror(errno));
	    }
	}
	free(hname);
...
int do_single_substitution(const char* sub_name_start,
	...
	*colon_ptr = '\0';
	...
	if (irec == NULL) {
	    if (output_warnings) {
		TE_Warning("subsitution %s:%s failed - no such instance",
			   sub_name, colon_ptr + 1);
	    }
	    return -1;
	} else {
	    ...
	    if (my_sub == NULL) {
		if (output_warnings) {
		    TE_Warning("subsitution %s:%s failed - no such port binding",
			       sub_name, colon_ptr + 1);
		}
		return -1;
	    } else {
		int len = strlen(my_sub);
		return output_substitution(output_buffer, slash_ptr,
					   sub_name, my_sub, len);
	    }
	}
    } else {
	...
	if (r == 0 || val == NULL) {
	    ...
	    return output_substitution(output_buffer, slash_ptr,
				       sub_name, (const char*) val, len);
	}
    }
}


int do_substitutions(const char* src, int len, sio_buffer* output,
		     int* errors_ptr) {
...
//    one
int process_defs_line(sio_buffer* defs_buf, char* line_end, int line_num) {
	...
	TE_Warning("Malformed entry at line %d in defs_file %s",
		   line_num, defs_file);
	return 1;
    }
    line_start = name_end;
...
int load_defs_file() {
    ...
    if (defs_fd < 0) {
	TE_Error("Could not open defs file %s : %s",
		 defs_path, strerror(errno));
	return -1;
    }
...
int read_log_parser_results(int fd, TestResult* results) {
...
int run_log_parser(TestResult* results, const char* output_file,
		   const char* test_case_name, bool html_output) {
    ...
    if (r < 0) {
	TE_Error("Can not execute log parser %s : %s",
		 parser_path, strerror(errno));
	free(parser_path);
	return -1;
    } else {
	free(parser_path);
...
int start_log_file() {
    ...
    if (fd < 0) {
	TE_Fatal("Could not open local log file %s : %s",
		 ud_info->log_file, strerror(errno));
	return -1;
    } else {
	log_file_fd = fd;
	return 0;
...
int send_log_collator_roll(const char* test_name) {
	...
	if (raf_result_code &&
	    ...
	    TE_Error("log roll failed : %s",
		     (response.length() >= 3) ? response[2] : "no_err_msg");
	    return -1;
	}
    }
}
...
int start_log_collator() {
	...
	r = execl(log_collate_path, log_collate_path,
		  "-p", port_str, "-L", ud_info->log_file, NULL);
...
void prep_and_run_log_parser(TestResult* tr, const char* test_case_name) {
...
void find_and_run_tests() {

    // Start by loading the test group file
    char test_group_path[1024];
    snprintf(test_group_path, 1023, "%s/%s",
		 defs_dir, test_group_file);
    ...
    if (test_group[0] != '\0') {
	if (r != 0) {
	    TE_Error("Can not run test_group '%s' since loading the group file failed",
		     test_group);
	    return;
	}
	...
	while ((cur_test_case = test_group_next(giter)) != NULL) {
	    Debug("group", "Running test_case %s from group",
		  cur_test_case->name);
...
void write_message_to_log_viewer(sio_buffer* write_buf) {
...
UserDirInfo* setup_user_and_dir_info() {
    uid_t my_uid = getuid();
...
void setup_port_stuff(UserDirInfo* ur) {

    ur->port = control_port;

    if (log_collator_port > 0) {
	ur->log_collator_arg = (char*) malloc(strlen(ur->hostname) + 32 + 1);
	sprintf(ur->log_collator_arg, "%s:%d",
		(ur->ip_str) ? ur->ip_str : ur->hostname, 
		log_collator_port);
    }
}
...
pid_t reap_and_kill_child(pid_t child_pid, int* exit_status) {
    ...
    pid_t reaped_pid = 
	reap_child(child_pid, exit_status, 30*1000);
...
void init_dir_stuff() {
	...
	if (errno == EACCES) {
	    ...
	    TE_Fatal("Unable to check access permissions for stuff dir %s : %s",
		     stuff_path, strerror(errno));
	}
    }
    ...
    if (rmsg) {
	TE_Fatal("no stuff dir : %s : %s %s", ud_info->test_stuff_path_and_dir,
		 rmsg, strerror(error_code));
    }
    ...
    if (rmsg) {
	Fatal("no slog dir : %s : %s %s", ud_info->log_dir,
	      rmsg, strerror(error_code));
    }
    ...
    if (rmsg) {
	TE_Fatal("no tmp dir : %s : %s %s", ud_info->tmp_dir,
		 rmsg, strerror(error_code));
    }
...
int main(int argc, char** argv) {
	...
	if (r == 0) {
	    Debug("log", "log collator started up; pid %d, port %d",
		  log_collator_pid, log_collator_port);
