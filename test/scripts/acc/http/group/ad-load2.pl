...
#  Author: Sophie Gu
#
#  $Id: ad-load2.pl,v 1.2 2003-06-01 18:38:31 re1 Exp $
#
...
my $CVS_ID_TAG   = '$Id: ad-load2.pl,v 1.2 2003-06-01 18:38:31 re1 Exp $';
...
our @ldaptest_create_args = ("package", "ldaptest",
			     "config", "-P %%(ts1) -p %%(ts1:tsHttpPort) -c 300 -i 1000");

our @ldaptest_start_args1 = ("args", "-a 1.0");
our @ldaptest_start_args2 = ("args", "-a 0.9 -e 0.1 -g 0.1 --cdrop_rate -0.1");
...
# This should work for all users.    
    $pcfg->LDAP (
		 ...
		 'enabled' => 1,
		 'query-timeout' => '30',
		 'base-dn' => "ou=SG1,ou=TG1,dc=my-domain,dc=com",
		 'server-name' => "$LDAP_SERVER",
		 'uid-filter' => "uid",
		 'bind-dn' => "cn=manager,dc=my-domain,dc=com",
		 'bind-pwd' => "secret",
		 'attribute-name' => 'cn',
		 'attribute-value' => 'SG1TG1',
		 );

# This should only authenticate users in ou=SG1.    
    $pcfg->LDAP (
		 'name' => "ldap-2",
		 'enabled' => 1,
		 'query-timeout' => '3',
		 'base-dn' => "ou=TG1,dc=my-domain,dc=com",
		 'server-name' => "$LDAP_SERVER",
		 'uid-filter' => "uid",
		 'bind-dn' => "cn=manager,dc=my-domain,dc=com",
		 'bind-pwd' => "secret",
...
# This should work for all users.
    $pcfg->LDAP (
		 ...
		 'enabled' => 1,
		 'query-timeout' => '3',
		 'base-dn' => "ou=SG2,ou=TG2,dc=my-domain,dc=com",
		 'server-name' => "$LDAP_SERVER",
		 'uid-filter' => "uid",
		 'bind-dn' => "cn=manager,dc=my-domain,dc=com",
		 'bind-pwd' => "secret",
...
# This should work for all users.
    $pcfg->LDAP (
		 ...
		 'enabled' => 1,
		 'query-timeout' => '3',
		 'base-dn' => "ou=SG3,ou=TG2,dc=my-domain,dc=com",
		 'server-name' => "$LDAP_SERVER",
		 'uid-filter' => "uid",
		 );
