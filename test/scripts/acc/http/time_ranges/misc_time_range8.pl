...
#
#
#  $Id: misc_time_range8.pl,v 1.2 2003-06-01 18:38:32 re1 Exp $
#
...
add CONFIG proxy.config.diags.debug.tags STRING policy.*|acc.*
...
dest_ip=0.0.0.0-255.255.255.255 time=&&(eval|{my \$t=time; my (\$s1,\$m1,\$h1)=localtime(\$t); my (\$s2,\$m2,\$h2)=localtime(\$t+3600); "100:00-\$h2:\$m2"}|) action=deny
EOF
...
@http_syntest_start_args = ("args","-f future_time_ranges.cfg -c FutureTimeRange");
