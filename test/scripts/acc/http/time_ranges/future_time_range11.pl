#
#  future_time_range11.pl (reverse proxy)
#
#    Run the default syntest testing suite against Traffic Server
#
#
#  $Id: future_time_range11.pl,v 1.2 2003-06-01 18:38:32 re1 Exp $
#
...
dest_ip=0.0.0.0-255.255.255.255 time=&&(eval|{my \$t=time; my (\$s1,\$m1,\$h1)=localtime(\$t+3600); my (\$s2,\$m2,\$h2)=localtime(\$t+7200); "\$h1:\$m1-\$h2:\$m2"}|) action=deny
EOF
...
@http_syntest_start_args =
    ("args", "-h %%(ts1) -p %%(ts1:tsHttpPort) -f future_time_ranges.cfg -c FutureTimeRangeRevProxy");
