#
#  gunzip_functional.pl
#
#   Functional testing for plugin gunzip
#
#  Author: franckc
#
#  $Id: 
...
# $cfg->set_debug_tags ('gunzip.*');
$cfg->add_config('plugin.config');
$cfg->add_config_line ('gunzip.so');
...
@syntest_start_args = ("args", "-f gunzip_tests.cfg -c Gunzip -noquit");
@empty_args = ();
...
TestExec::put_instance_file_raw("ts1", $plg_dst . "gunzip.so", $plg_src . "gunzip.so",);
...
# Install
TestExec::put_instance_file_raw("syntest1", "gunzip_tests.cfg",   "gunzip_tests.cfg");
TestExec::put_instance_file_raw("syntest1", "gunzip_syntest.tar", "gunzip_syntest.tar");
