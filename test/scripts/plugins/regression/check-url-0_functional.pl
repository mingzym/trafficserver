...
# $cfg->set_debug_tags ('check-url-0');
$cfg->add_config('plugin.config');
$cfg->add_config_line ('check-url-0.so');
...
# Install the plugin
$plg_src = "../../../../proxy/api/samples/internal/check-url/";
$plg_dst = "config/plugins/";
$cfg_src = "config/";
$cfg_dst = "config/plugins/";

TestExec::put_instance_file_raw
    ("ts1", $plg_dst . "check-url-0.so",  
     $plg_src . "check-url-0.so",);
