#
#  exercise-headers_functional.pl
#
#    Run functional test cases for internal QA regression
#    plugin exercise-header
#
#    The plugin exercises request and request headers
...
# $cfg->set_debug_tags ('exercise-header');
$cfg->add_config('plugin.config');
$cfg->add_config_line ('exercise-headers.so');
...
# Install the plugin
$plg_src = "../../../../proxy/api/samples/regression/exercise-headers/";
$plg_dst = "config/plugins/";
$cfg_src = "config/";
$cfg_dst = "config/plugins/";

TestExec::put_instance_file_raw
    ("ts1", $plg_dst . "exercise-headers.so",  
     $plg_src . "exercise-headers.so",);
