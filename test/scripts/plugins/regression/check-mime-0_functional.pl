#
#  check-mime-0_functional.pl
#
#    Run functional test cases for internal QA regression
#    plugin check-mime-0
#
#    The plugin exercises request headers
...
# $cfg->set_debug_tags ('check-mime-0');
$cfg->add_config('plugin.config');
$cfg->add_config_line ('check-mime-0.so');
...
# Install the plugin
$plg_src = "../../../../proxy/api/samples/internal/check-mime/";
$plg_dst = "config/plugins/";
$cfg_src = "config/";
$cfg_dst = "config/plugins/";

TestExec::put_instance_file_raw
    ("ts1", $plg_dst . "check-mime-0.so",  
     $plg_src . "check-mime-0.so",);
