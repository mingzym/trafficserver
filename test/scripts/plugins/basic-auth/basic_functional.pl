...
#  basic_functional.pl
#
#   Functional testing for plugin basic-auth
#
#  Author: franckc
#
#  $Id: 
...
# $cfg->set_debug_tags ('basic.*');
$cfg->add_config('plugin.config');
$cfg->add_config_line ('basic-auth.so');
...
@syntest_start_args = ("args", "-f basic_tests.cfg -c Basic-Auth -noquit");
@empty_args = ();
...
# Install the plugin
$plg_src = "../../../../proxy/api/samples/basic-auth/";
$plg_dst = "config/plugins/";
$cfg_src = "config/";
$cfg_dst = "config/plugins/";
TestExec::put_instance_file_raw("ts1", $plg_dst . "basic-auth.so", $plg_src . "basic-auth.so",);
...
# Install
TestExec::put_instance_file_raw("syntest1", "basic_tests.cfg", "basic_tests.cfg");
TestExec::put_instance_file_raw("syntest1", "basic_syntest.tar", "basic_syntest.tar");
