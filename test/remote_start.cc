/****************** -*- Mode: C++; Indent: Inktomi4 -*- *********************
   ...
   $Id: remote_start.cc,v 1.2 2003-06-01 18:38:29 re1 Exp $
...
static int create_pipe_for_remote(const char* hostname, int* pipe_arg, int in_pipe) {
    ...
    do {
	r = fcntl ((in_pipe == true) ? pipe_arg[0] : pipe_arg[1],
		   F_SETFL, O_NONBLOCK);
    } while (r < 0 && errno == EINTR);
...
pid_t start_remote_shell(const UserDirInfo* ud_info, const char* hostname,
...
const char* send_remote_cmd(sio_buffer* in_buf, int fd_in,
			    sio_buffer* out_buf, int fd_out) {
...
const char* determine_arch(int fd_in, int fd_out, char** arch_out) {
...
//
char* find_proc_manager_binary(const UserDirInfo* ud, const char* remote_arch) {
...
const char* setup_remote_directories(int fd_in, int fd_out, UserDirInfo* ud) {

    sio_buffer in_buf;
    sio_buffer out_buf;

    const char check_path_format[] =
	"if [ -d %s ]; then\n"
	"   echo ok\n"
	"else\n"
	"   echo not found\n"
    ...
    const char* rmsg = 
	send_remote_cmd(&in_buf, fd_in, &out_buf, fd_out);
    ...
    *end = '\0';
	...
	int len = sizeof(mkdir_format) +
	    strlen(ud->test_stuff_path_and_dir) + 1;
	char* cmd = (char*) malloc(len);
...
const char* transfer_proc_manager_binary(int local_file_fd, int fd_in, int* timeout_ms) {
...
const char* check_remote_proc_manager(int fd_in, int fd_out,
				      UserDirInfo* ud,
				      const char* remote_proc_mgr_name,
				      const char* remote_arch,
				      int* ok)
{
    sio_buffer in_buf;
    sio_buffer out_buf;
	...
	*ok = 1;
	...
	TE_Warning("stat on for proc_manager % : %s",
		local_pm, strerror(errno));
	*ok = 1;
    } else {
...
//
//
const char* start_remote_proc_manager(int& fd_in, int& fd_out,
				      UserDirInfo* ud,
				      const char* remote_proc_mgr_name,
                      int remote_proc_mgr_killtm) {
    sio_buffer in_buf;
    sio_buffer out_buf;
    ...
    sprintf(cmd, start_format, remote_proc_mgr_name, ud->port,
	    ...
	    (ud->log_collator_arg == NULL) ? "" : ud->log_collator_arg,
        remote_proc_mgr_killtm);
...
//
//
const char* put_and_start_proc_manager(int& fd_in, int& fd_out,
				       UserDirInfo* ud,
				       const char* remote_proc_mgr_name,
				       const char* remote_arch,
                       int remote_proc_mgr_killtm) {
    ...
    const char put_and_start_format[] =
	"cat - > %s; chmod 0755 %s; ./%s -r -q -d . -p %d%s%s -k %d\n";
...
const char* handle_proc_manager(int& fd_in, int& fd_out,
				UserDirInfo* ud, const char* arch, int kw) {
    ...
    const char chdir_format[] =
	"if cd %s\n"
        "then\n"
	"   echo ok\n"
	"fi\n";

    const char check_file_format[] =
	"if [ -e %s ]; then\n"
	"   echo ok\n"
	"else\n"
	"   echo not found\n"
    ...
    *end = '\0';
    Debug("remote", "Response to check for %s : %s",
	  proc_manager_name, out_buf.start());

    if (strcmp(out_buf.start(), "ok") == 0) {
	int ok = 0;
	rmsg = check_remote_proc_manager(fd_in, fd_out, ud,
					 proc_manager_name, arch, &ok) ;
	...
	if (ok) {
	    ...
	    rmsg = start_remote_proc_manager(fd_in, fd_out, ud,
					     proc_manager_name, kw);
	} else {
	    Debug("remote",  "proc_manager is out of date on remote");
	    rmsg = put_and_start_proc_manager(fd_in, fd_out, ud,
					      proc_manager_name, arch, kw);
	}
...
const char* check_remote_isalive(unsigned int ip, int port, int retries=20) {
