/****************** -*- Mode: C++; Indent: Inktomi4 -*- *********************
   ...
   $Id: sio_loop.cc,v 1.2 2003-06-01 18:38:29 re1 Exp $
...
int SIO::open_server(unsigned short int port) {
    struct linger lngr;
    int sock;
    int one = 1;
    int err = 0;
...
int SIO::make_client (unsigned int addr, int port) {
    struct linger lngr;
...
void S_Continuation::handle_event(s_event_t event, void* data) {
...
int FD_Handler::clear_non_block_flag() {
    if (fd >= 0) {
	int flags;
...
int FD_Handler::set_linger(int on, int ltime) {
	...
	if (setsockopt (fd, SOL_SOCKET, SO_LINGER, (char*) &linfo,
			sizeof (struct linger)) < 0) {
	    return -errno;
	} else {
	    return 0;
...
void SIO::add_fd_handler(FD_Handler* fdh) {
    fd_list.push(fdh);
    num_active_fd++;
}

void SIO::remove_fd_handler(FD_Handler* fdh) {
    fd_list.remove(fdh);
    num_active_fd--;
}
...
S_Event* SIO::schedule_in(S_Continuation* c, int ms) {
...
void update_next_etime(ink_hrtime now) {
...
void SIO::add_exit_handler(S_Continuation* c) {
    exit_handler = c;
}
