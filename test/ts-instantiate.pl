...
#    framework
#
#  $Id: ts-instantiate.pl,v 1.2 2003-06-01 18:36:51 re1 Exp $
#
...
sub process_other_file_blob {
    my ($file_name) = @_;
    my $file_path = $run_dir . "/config/" . $file_name;
...
sub output_remap_config {
    my $remap_file = $run_dir . "/config/remap.config";
...
sub output_ftp_remap_config {
    my $ftp_remap_file = $run_dir . "/config/ftp_remap.config";
...
sub output_ipnat_conf {
    my $ipnat_file = $run_dir . "/config/ipnat.conf";
...
sub output_filter_config {
    my $filter_file = $run_dir . "/config/filter.config";
...
our @populate_dirs = ( "bin", "config", "config/internal", "logs");
...
( "bin/traffic_server" => "bin/traffic_server",
  ...
  "config/body_factory" => "config/body_factory",
...
sub populate_run_dir() {
    ...
    my $config_dir_source = $bin_dir . "/config";
    my $config_dir_target = $run_dir . "/config";
...
if ($bin_dir) {
    $records_config_in = $bin_dir . "/config/records.config";
} else {
...
if ($no_run_dir) {
    $records_config_out = $bin_dir . "/config/records.config.shadow";
    $run_dir = $bin_dir;
} else {
    $records_config_out = $run_dir . "/config/records.config";
}
